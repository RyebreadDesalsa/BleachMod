...
{
    internal class NozarashiSwing : ModProjectile
    {
		...
		// We define timing functions for each stage
		// Note that you can change this to suit the need of your projectile
		private float prepTime => 1f;
		private float execTime => 12f;
		private float hideTime => 12f;

		public override string Texture => "BleachMod/Content/Items/Weapons/ShinigamiSwords/NozarashiBankai"; // Use texture of item as projectile texture
		...
		public override void SetDefaults()
		{
			...
			Projectile.DamageType = ModContent.GetInstance<ShinigamiDamage>(); // Projectile is a Shinigami projectile
		}
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			{
				origin = new Vector2(Projectile.width+25, Projectile.height);
		...
		public override bool PreAI()
		{
			...
			if (player.direction == -1)
			{
				...
				Projectile.Center = player.Center + new Vector2(-5f, -85f);
			}
			else
			{
			...
			if (start)
            {
				player.SetCompositeArmFront(true,0,MathHelper.ToRadians(180));
            ...
            {
				rotation += 1f;
				player.SetCompositeArmFront(true, 0, MathHelper.ToRadians(rotation));
				Projectile.velocity = Projectile.velocity.RotatedBy(MathHelper.ToRadians(1f));
				Projectile.rotation += MathHelper.ToRadians(1f);
				Vector2 center = new Vector2(player.Center.X,player.Center.Y);
